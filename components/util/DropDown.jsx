...
const DropDown = ({cities,id,register}) => {
  ...
  return (
    ...
    <div  className='w-full '>
        <div onClick={()=>setIsOpen(!isOpen)} className='flex flex-row justify-between items-center w-full rounded border p-2 mt-1 cursor-pointer'>
            <input {...register(id)} disabled value={selected.title} className='cursor-pointer bg-white w-full outline-none text-xs'/>
            ...
            isOpen && (
                ...
                <div className='bg-white border rounded shadow w-full overflow-y-scroll mt-1 h-60v'>
                    {citiesList.map((ci,index)=>(
                        <div key={index}  onClick={()=>setSelected(cities[index])} className='flex flex-row justify-between items-center hover:bg-stone-100 cursor-pointer'>
